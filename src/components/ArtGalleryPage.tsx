...
import { subscribeToArtworks, toggleArtworkLike, isArtworkLikedLocally } from '../services/artworksApi';
import { playUiClick, playSparkleSound, playHoverTick, playHeartPopSound } from '../utils/audio';
...
export const ArtGalleryPage: React.FC<ArtGalleryPageProps> = ({
  ...
  const [sortOrder, setSortOrder] = useState<'newest' | 'most_liked'>('newest');
  ...
  // Sync local liked statuses from localStorage
  useEffect(() => {
    const initialMap: Record<string, boolean> = {};
    artworks.forEach((art) => {
      initialMap[art.id] = isArtworkLikedLocally(art.id);
    });
  ...
  // Sort artworks based on chosen order
  ...
  const handleToggleLike = async (artwork: Artwork, e?: React.MouseEvent) => {
    if (e) {
      e.stopPropagation();
    }
    playHeartPopSound(soundEnabled);

    const isCurrentlyLiked = !!likedMap[artwork.id];
    const nextLiked = !isCurrentlyLiked;
    const currentLikes = Number(artwork.likesCount || 0);
    const nextLikes = Math.max(0, currentLikes + (nextLiked ? 1 : -1));

    // Show heart particle burst effect
    if (nextLiked) {
      setHeartBurstId(artwork.id);
      setTimeout(() => {
        setHeartBurstId((prev) => (prev === artwork.id ? null : prev));
      }, 700);
    }

    // Optimistic UI update
    setLikedMap((prev) => ({ ...prev, [artwork.id]: nextLiked }));
    setArtworks((prev) =>
      prev.map((item) =>
        item.id === artwork.id ? { ...item, likesCount: nextLikes } : item
      )
    );
    if (activeLightboxArtwork && activeLightboxArtwork.id === artwork.id) {
      setActiveLightboxArtwork((prev) =>
        prev && prev.id === artwork.id ? { ...prev, likesCount: nextLikes } : prev
      );
    }

    // Server / Firestore synchronization
    try {
      const result = await toggleArtworkLike(artwork.id, isCurrentlyLiked, currentLikes);
      if (result && result.success) {
        setLikedMap((prev) => ({ ...prev, [artwork.id]: result.isLiked }));
        setArtworks((prev) =>
          prev.map((item) =>
            item.id === artwork.id ? { ...item, likesCount: result.likesCount } : item
          )
        );
        if (activeLightboxArtwork && activeLightboxArtwork.id === artwork.id) {
          setActiveLightboxArtwork((prev) =>
            prev && prev.id === artwork.id
              ? { ...prev, likesCount: result.likesCount }
              : prev
          );
        }
      }
    } catch (err) {
      console.warn('[handleToggleLike] sync failed:', err);
    }
  };
  ...
  return (
    ...
    >
      ...
      <main className="relative z-10 flex-1 max-w-7xl w-full mx-auto px-2.5 sm:px-6 py-3 sm:py-6">
        ...
        {/* 4. GALLERY SORT & CONTROL BAR */}
        <div className="flex flex-wrap items-center justify-between gap-2 max-w-7xl mx-auto px-1 sm:px-2 mb-3 sm:mb-4">
          <div className="text-[11px] sm:text-xs font-bold text-slate-500 dark:text-purple-300">
            Đang hiển thị <span className="font-black text-amber-500">{sortedArtworks.length}</span> tác phẩm
          </div>

          <div className="flex items-center gap-1.5 p-1 rounded-xl sm:rounded-2xl bg-white/70 dark:bg-black/40 border border-slate-200 dark:border-white/10 backdrop-blur-md shadow-xs">
            <button
              type="button"
              onClick={() => {
                playUiClick(soundEnabled);
              ...
              className={`px-2.5 sm:px-3 py-1 rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-black transition-all cursor-pointer ${
                sortOrder === 'newest'
            ...
            <button
              ...
              onClick={() => {
                playUiClick(soundEnabled);
                setSortOrder('most_liked');
              }}
              className={`px-2.5 sm:px-3 py-1 rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-black transition-all cursor-pointer flex items-center gap-1 ${
        ...
        {/* 5. ARTWORK SHOWCASE GRID (Optimized with Chunked Rendering & Lazy Loading) */}
              ...
              {sortedArtworks.slice(0, visibleCount).map((artwork) => (
                ...
                >
                  ...
                  <div className="relative w-full aspect-square rounded-lg sm:rounded-2xl overflow-hidden bg-black/10 mb-2 sm:mb-3 border">
                    ...
                    {/* Heart / Like Button on Card Image */}
                    <button
                      type="button"
                      onClick={(e) => handleToggleLike(artwork, e)}
                      title={likedMap[artwork.id] ? 'Bỏ tim' : 'Thả tim cho tác phẩm'}
                      className={`absolute top-1.5 right-1.5 sm:top-2 sm:right-2 z-30 px-2 py-0.5 sm:py-1 rounded-lg sm:rounded-xl text-[9px] sm:text-[11px] font-black border backdrop-blur-md flex items-center gap-1 transition-all cursor-pointer shadow-md active:scale-90 ${
                        likedMap[artwork.id]
                          ? 'bg-rose-500/95 text-white border-rose-300 shadow-[0_0_12px_rgba(244,63,94,0.55)]'
                          : 'bg-black/65 hover:bg-black/85 text-white/90 border-white/30 hover:border-pink-300'
                      }`}
                    >
                      <Heart
                      ...
                      {heartBurstId === artwork.id && (
                        <span className="absolute -top-3 left-1/2 -translate-x-1/2 text-rose-300 animate-ping text-[10px] pointer-events-none">
                          ❤️
                        </span>
                      )}
                    </button>
                  ...
                  <div className="space-y-0.5 sm:space-y-1 text-left px-0.5 sm:px-1">
                    ...
                    <div className="flex items-center justify-between text-[10px] sm:text-[11px] pt-1 border-t border-slate-100 dark:border-white/5">
                      {/* Artist name */}
                      <div className="flex items-center gap-1 min-w-0 text-slate-500 dark:text-purple-300">
                        ...
                        <span className="truncate font-bold max-w-[65px] sm:max-w-[100px]">{artwork.authorName}</span>
                      </div>

                      {/* Heart Count & Formatted Date */}
                      <div className="flex items-center gap-1.5 shrink-0">
                        <span className="flex items-center gap-0.5 text-rose-500 dark:text-rose-400 font-bold text-[10px] sm:text-[11px]">
                          <Heart className={`w-2.5 h-2.5 sm:w-3 sm:h-3 ${likedMap[artwork.id] ? 'fill-rose-500' : ''}`} />
                          <span>{artwork.likesCount || 0}</span>
                        </span>
            ...
            {sortedArtworks.length > visibleCount && (
              <div className="flex justify-center pt-4 pb-2">
                ...
                >
                  ...
                  <span>Xem thêm tranh ({sortedArtworks.length - visibleCount} tác phẩm còn lại)</span>
                </button>
              </div>
            )}
          </div>
          ...
          >
            ...
            >
              ...
              <div className="w-full mt-2.5 sm:mt-3 p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-white/10 backdrop-blur-xl border border-white/20 text-white flex flex-col sm:flex-row items-center justify-between gap-2.5 sm:gap-3 text-center sm:text-left">
                ...
                {/* Lightbox Heart Action Button */}
                <div className="flex items-center gap-2.5 sm:gap-3 shrink-0">
                  <button
                    id="btn-lightbox-like"
                    type="button"
                    onClick={(e) => handleToggleLike(activeLightboxArtwork, e)}
                    className={`relative group px-4 sm:px-5 py-2 sm:py-2.5 rounded-2xl font-black text-xs sm:text-sm border-2 shadow-xl flex items-center gap-2 transition-all cursor-pointer active:scale-95 ${
                      likedMap[activeLightboxArtwork.id]
                        ? 'bg-gradient-to-r from-rose-500 to-pink-600 hover:from-rose-600 hover:to-pink-700 text-white border-pink-300 shadow-[0_0_20px_rgba(244,63,94,0.45)]'
                        : 'bg-white/15 hover:bg-white/25 text-white border-white/30 hover:border-pink-300'
                    }`}
                  >
                    <Heart
                    ...
                    />
                    <span>
                      {likedMap[activeLightboxArtwork.id] ? 'Đã thả tim' : 'Thả tim'}
                    </span>
                    <span className="px-2 py-0.5 rounded-full text-[10px] sm:text-xs font-black bg-black/40 border border-white/20">
                      {activeLightboxArtwork.likesCount || 0}
                    </span>

                    {heartBurstId === activeLightboxArtwork.id && (
                      <motion.div
                        initial={{ scale: 0.6, opacity: 1, y: 0 }}
                        animate={{ scale: 1.6, opacity: 0, y: -30 }}
                        transition={{ duration: 0.7 }}
                        className="absolute -top-4 left-1/2 -translate-x-1/2 text-rose-300 font-black pointer-events-none text-sm sm:text-base flex items-center gap-1"
                      >
                        💖 +1
                      </motion.div>
                    )}
